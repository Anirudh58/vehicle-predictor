...
from .bilinear_vgg16 import BCNN_vgg16_fc, BCNN_vgg16_ft
from .bilinear_resnet18 import BCNN_resnet18_fc, BCNN_resnet18_ft
from .bilinear_resnet50 import BCNN_resnet50_fc, BCNN_resnet50_ft
...
def get_model(num_classes, backbone_model):
    ...
    if backbone_model == 'bilinear_vgg16_fc':
        model = BCNN_vgg16_fc(num_classes, pretrained=True)

    if backbone_model == 'bilinear_vgg16_ft':
        model = BCNN_vgg16_ft(num_classes)

    if backbone_model == 'bilinear_resnet18_fc':
        model = BCNN_resnet18_fc(num_classes, pretrained=True)

    if backbone_model == 'bilinear_resnet18_ft':
        model = BCNN_resnet18_ft(num_classes)

    if backbone_model == 'bilinear_resnet50_fc':
        model = BCNN_resnet50_fc(num_classes, pretrained=True)

    if backbone_model == 'bilinear_resnet50_ft':
        model = BCNN_resnet50_ft(num_classes)
    
    return model

def train_model(model, model_info, dataset_sizes, dataloader_train, dataloader_val, criterion, optimizer, scheduler, target_labels, num_epochs=25):
    since = time.time()
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if epoch % 10 == 0:
                ...
                torch.save(best_model_wts, os.path.join(root_path, 'models', f"{model_name}_acc_{best_acc:.3f}.pth"))
